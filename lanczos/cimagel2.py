...
def lanczos_digital_zoom(image, zoom_factor):
    ...
    lut = InitializeLanczosIntLUT(2)  # Use Lanczos-2 interpolation
            ...
            for i in range(-1, 3):
                for j in range(-1, 3):
                    ...
                    if 0 <= px < cropped_image.shape[1] and 0 <= py < cropped_image.shape[0]:
                        lanczos_weight = LanczosInterpolation(frac_x - i, lut, 2) * LanczosInterpolation(frac_y - j, lut, 2)
...
if __name__ == "__main__":
    ...
    loop_time = timeit.timeit(lambda: lanczos_digital_zoom(original_image, zoom_factor), number=1)
    print(f"Loop time: {loop_time:.6f} seconds")

    cv2.imwrite("single_lanczos2.jpg", zoomed_image)
